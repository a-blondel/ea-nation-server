...
/**
 * Simple Stream Cipher 2
 */
public class CryptSSC2 {

    static final int[] _Cipher_Crc32 = new int[]{
    ...
    private static boolean _SSC2_Init = false;
    ...
    public static void cryptSSC2Init(CryptSSC2T pState, byte[] pKey, int iKey, int iIter) {
        ...
        if (!_SSC2_Init) {
            _SSC2_Init = true;
            cryptSSC2Init(_SSC2_Random, "hello world".getBytes(), -1, 10);
        }
    ...
    public static void cryptSSC2Apply(CryptSSC2T pState, byte[] pData, int iData) {
        byte uSwap0, uSwap1;
        byte iIdx = pState.getIdx();
        long uCrc = pState.getCrc();

        // process each data byte
        for (; iData > 0; --iData) {
            // put sbox data into crc
            uCrc = ((uCrc >> 8) ^ _Cipher_Crc32[(int) (uCrc & 0xFF) ^ (pState.getSbox()[iIdx++ & 0xFF] & 0xFF)]) & 0xFFFFFFFFL;

            // swap the s-boxes entries
            uSwap0 = pState.getSbox()[iIdx & 0xFF];
            uSwap1 = pState.getSbox()[(int) (uCrc & 0xFF)];
            pState.getSbox()[iIdx & 0xFF] = uSwap1;
            pState.getSbox()[(int) (uCrc & 0xFF)] = uSwap0;

            // use swap data to index into sboxes
            pData[iData - 1] ^= pState.getSbox()[(uSwap0 - uSwap1) & 0xFF];
        }
    ...
    public static int cryptSSC2StringEncrypt(byte[] pDst, int iLen, byte[] pSrc, byte[] pKey, int iKey, int iIter) {
        ...
        cryptSSC2Init(_SSC2_Random, "ru paranoid?".getBytes(), -1, -1);
        ...
        for (int i = 0; i < iLen - 1; ++i) {
            ...
            cryptSSC2Apply(ref, uEncArray, 1);
            uEnc = uEncArray[0];

            // encode the data
            iSum = (96 - 32) + Byte.toUnsignedInt(uDat) + (Byte.toUnsignedInt(uEnc) % 96);
    ...
    public static int cryptSSC2StringDecrypt(byte[] pDst, int iLen, byte[] pSrc, byte[] pKey, int iKey, int iIter) {
        ...
        for (iIdx = 0; iIdx < iLen - 1; ++iIdx) {
            ...
            cryptSSC2Apply(ref, uDecArray, 1);
            uDec = uDecArray[0];

            // decode the data
            iSum = (96 - 32) + Byte.toUnsignedInt(uDat) - (Byte.toUnsignedInt(uDec) % 96);
    ...
    @Getter
    @Setter
    public static class CryptSSC2T {
        public byte idx;
        public long crc;
